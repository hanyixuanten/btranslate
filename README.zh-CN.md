...
**子目录** 模式会解析配置语言对应的路径，例如 `/en/example-post/`。**子域名** 模式通过主机名映射语言：在设置中每行填写一条 `domain=language`。两种模式可以同时启用：已绑定域名的语言使用该域名，未绑定域名的目标语言使用子目录，从而确保每种语言只有一种 URL 策略并避免重复内容。只启用子目录模式时，域名绑定字段不会显示。
